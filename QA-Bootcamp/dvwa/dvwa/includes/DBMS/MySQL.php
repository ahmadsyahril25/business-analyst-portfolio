...
// Add account_enabled column to users table
...
// Create documents table for IDOR vulnerability
...
if( !mysqli_query($GLOBALS["___mysqli_ston"], $create_documents) ) {
    dvwaMessagePush( "Could not create documents table<br />SQL: " . ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)) );
    dvwaPageReload();
}
dvwaMessagePush( "'documents' table was created." );

// Insert sample documents for each user
$insert_docs = "INSERT INTO documents (user_id, title, content) VALUES
    (1, 'Admin Secret Document', 'This is a confidential admin document with sensitive information. Only admin should see this.'),
    (1, 'Server Configuration', 'Database password: super_secret_pass123. API key: sk-admin-12345abcdef.'),
    (2, 'Gordon\'s Project Notes', 'Project deadline is next Friday. Need to finalize the budget report.'),
    (2, 'Meeting Minutes', 'Attendees: John, Sarah, Mike. Discussed Q4 targets and marketing strategy.'),
    (3, 'Personal Diary', 'Today was a good day. Found a new vulnerability to test.'),
    (3, 'Bug Report', 'Found XSS vulnerability in the search field. Needs fixing.'),
    (4, 'Art Collection List', 'Paintings to acquire: Starry Night, The Scream, Guernica.'),
    (4, 'Gallery Opening', 'Scheduled for next month. Invitations sent to 150 guests.'),
    (5, 'Shopping List', 'Milk, eggs, bread, cheese, coffee.'),
    (5, 'Weekend Plans', 'Going hiking on Saturday. BBQ on Sunday with family.')";

if( !mysqli_query($GLOBALS["___mysqli_ston"], $insert_docs) ) {
    dvwaMessagePush( "Could not insert sample documents<br />SQL: " . ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)) );
    dvwaPageReload();
}
dvwaMessagePush( "Sample documents inserted for IDOR testing." );

// Create idor_log table for logging IDOR access attempts
$create_idor_log = "CREATE TABLE idor_log (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    doc_id INT NOT NULL,
    action VARCHAR(50) NOT NULL,
...
if( !mysqli_query($GLOBALS["___mysqli_ston"], $create_idor_log) ) {
    dvwaMessagePush( "Could not create idor_log table<br />SQL: " . ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)) );
    dvwaPageReload();
}
dvwaMessagePush( "'idor_log' table was created." );
